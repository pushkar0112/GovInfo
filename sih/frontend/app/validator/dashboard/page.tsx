...
type ValidatorTab = "assigned" | "history";

function ValidatorDashboardContent() {
  const { currentUser, logout } = useAuth();
  const [activeTab, setActiveTab] = useState<ValidatorTab>("assigned");
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <Badge variant="gov" className="text-[10px] bg-cyan-400/20 text-cyan-200 border-cyan-400/30">
                  Testing & Verification Agency
                </Badge>
              </div>
              <p className="text-xs text-slate-300">
                {currentUser.organization_name || "Accredited Testing Laboratory"} • Field Outcome Certification
              </p>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-8 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 text-xs text-slate-600 flex-wrap">
              ...
              <span>•</span>
              <span>Laboratory: {currentUser.organization_name || "Testing Agency"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex border-b border-slate-200 overflow-x-auto gap-2 text-xs font-semibold">
          {[
            { id: "assigned", label: "Assigned Validations", icon: ClipboardCheck },
            ...
            return (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as ValidatorTab)}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
          {activeTab === "assigned" && (
            <div className="space-y-4">
              <div>
                <h3 className="text-base font-bold text-slate-900">Assigned Pilot Sandbox Validations</h3>
                <p className="text-xs text-slate-500 mt-0.5">
                  Verify technical outcome metrics, run cybersecurity & load audits, and issue GFR compliance test certificates.
                </p>
              </div>
              <div className="p-12 text-center border border-dashed border-slate-200 rounded-xl flex flex-col items-center">
                ...
                <h4 className="text-sm font-bold text-slate-700">No pilot validations assigned yet.</h4>
                <p className="text-xs text-slate-500 mt-1 max-w-sm">
                  Active field sandbox pilots reaching completion will be assigned to your agency for independent verification.
                </p>
              </div>
            </div>
          )}
          ...
          {activeTab === "history" && (
            <div className="space-y-4">
              <div>
                <h3 className="text-base font-bold text-slate-900">Validation Certification History</h3>
                <p className="text-xs text-slate-500 mt-0.5">
                  Archived digital certificates, test sign-offs, and compliance audit reports.
                </p>
              </div>
              <div className="p-12 text-center border border-dashed border-slate-200 rounded-xl flex flex-col items-center">
                <History className="w-10 h-10 text-slate-300 mb-2" />
                <h4 className="text-sm font-bold text-slate-700">No past validation certificates issued yet.</h4>
                <p className="text-xs text-slate-500 mt-1 max-w-sm">
                  Completed validation records will be stored immutably to support subsequent GeM direct procurement decisions.
                </p>
              </div>
            </div>
          )}
